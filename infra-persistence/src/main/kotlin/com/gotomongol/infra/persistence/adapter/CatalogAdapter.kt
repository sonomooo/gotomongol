...
class SpotItemAdapter(private val repo: JpaSpotItemRepository) : SpotItemPort {
    override fun findAll(): List<SpotItem> { return repo.findAll().map { SpotItem(it.id, it.name, it.active) } }
    override fun findActive(): List<SpotItem> { return repo.findByActiveTrue().map { SpotItem(it.id, it.name, it.active) } }
    override fun save(item: SpotItem): SpotItem { val e = repo.save(SpotItemEntity(item.id, item.name, item.active)); return SpotItem(e.id, e.name, e.active) }
...
class ActivityItemAdapter(private val repo: JpaActivityItemRepository) : ActivityItemPort {
    override fun findAll(): List<ActivityItem> { return repo.findAll().map { ActivityItem(it.id, it.name, it.active) } }
    override fun findActive(): List<ActivityItem> { return repo.findByActiveTrue().map { ActivityItem(it.id, it.name, it.active) } }
    override fun save(item: ActivityItem): ActivityItem { val e = repo.save(ActivityItemEntity(item.id, item.name, item.active)); return ActivityItem(e.id, e.name, e.active) }
    override fun deleteById(id: Long) { repo.deleteById(id) }
}
